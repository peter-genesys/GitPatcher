  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PATCH_PREREQS_TAPI" IS
...
BEGIN
  ...
  l_pk_given := i_patch_prereqs.patch_prereq_id is not null;
  if l_pk_given then
    --Get Rowid from PK
    l_rowid := get_rowid(
  ...
  if not l_pk_given and l_rowid is null then
    --Get Rowid from patch_prereqs_uk1
    l_rowid := get_rowid (
...
BEGIN
  ...
  l_rowid := get_rowid(
      i_patch_prereqs  => i_patch_prereqs
     ,i_raise_error        => i_raise_error );

  l_patch_prereqs := patch_prereqs_rid (
     i_rowid               => l_rowid
    ,i_raise_error  => i_raise_error );
...
BEGIN
  ...
  INSERT INTO patch_prereqs VALUES io_patch_prereqs RETURNING rowid into io_rowid;

  --Retrieve the full record, as columns may have been changed by triggers.
  io_patch_prereqs := patch_prereqs_rid(i_rowid => io_rowid);
...
BEGIN
  ...
  ins(
    io_patch_prereqs  => io_patch_prereqs
   ,io_rowid             => l_rowid);
...
BEGIN
  ...
  UPDATE patch_prereqs SET ROW = io_patch_prereqs where rowid = io_rowid RETURNING rowid into io_rowid;
...
BEGIN
  ...
  --Get Rowid
  l_rowid := get_rowid(
      i_patch_prereqs  => io_patch_prereqs
     ,i_raise_error        => 'Y' );
...
BEGIN
  ...
  del(i_patch_prereqs => l_patch_prereqs
     ,i_raise_error       => i_raise_error);
