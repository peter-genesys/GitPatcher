...
END; 
  ...
  BEGIN
    ...
    FOR l_dba_objects_v IN (select *
                          from dba_objects_v 
                          where (owner like UPPER(i_schema)) 
                          and status ='INVALID' ) LOOP
      put_lines('INVALID '||l_dba_objects_v.object_type||': '||l_dba_objects_v.owner ||'.' || l_dba_objects_v.OBJECT_NAME);  
  ...
  FUNCTION get_dba_objects(i_owner       IN VARCHAR2
                          ...
                          ,i_object_type IN VARCHAR2) RETURN dba_objects_v%ROWTYPE IS
      ...
      l_dba_objects dba_objects_v%ROWTYPE;
