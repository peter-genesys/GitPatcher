...
BEGIN
  ...
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_ID','i') then io_new_rec.PATCH_ID := i_old_rec.PATCH_ID; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_NAME','i') then io_new_rec.PATCH_NAME := i_old_rec.PATCH_NAME; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'DB_SCHEMA','i') then io_new_rec.DB_SCHEMA := i_old_rec.DB_SCHEMA; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'BRANCH_NAME','i') then io_new_rec.BRANCH_NAME := i_old_rec.BRANCH_NAME; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'TAG_FROM','i') then io_new_rec.TAG_FROM := i_old_rec.TAG_FROM; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'TAG_TO','i') then io_new_rec.TAG_TO := i_old_rec.TAG_TO; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'SUPPLEMENTARY','i') then io_new_rec.SUPPLEMENTARY := i_old_rec.SUPPLEMENTARY; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_DESC','i') then io_new_rec.PATCH_DESC := i_old_rec.PATCH_DESC; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_COMPONANTS','i') then io_new_rec.PATCH_COMPONANTS := i_old_rec.PATCH_COMPONANTS; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_CREATE_DATE','i') then io_new_rec.PATCH_CREATE_DATE := i_old_rec.PATCH_CREATE_DATE; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_CREATED_BY','i') then io_new_rec.PATCH_CREATED_BY := i_old_rec.PATCH_CREATED_BY; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'NOTE','i') then io_new_rec.NOTE := i_old_rec.NOTE; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'LOG_DATETIME','i') then io_new_rec.LOG_DATETIME := i_old_rec.LOG_DATETIME; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'COMPLETED_DATETIME','i') then io_new_rec.COMPLETED_DATETIME := i_old_rec.COMPLETED_DATETIME; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'SUCCESS_YN','i') then io_new_rec.SUCCESS_YN := i_old_rec.SUCCESS_YN; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'RETIRED_YN','i') then io_new_rec.RETIRED_YN := i_old_rec.RETIRED_YN; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'RERUNNABLE_YN','i') then io_new_rec.RERUNNABLE_YN := i_old_rec.RERUNNABLE_YN; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'WARNING_COUNT','i') then io_new_rec.WARNING_COUNT := i_old_rec.WARNING_COUNT; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'ERROR_COUNT','i') then io_new_rec.ERROR_COUNT := i_old_rec.ERROR_COUNT; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'USERNAME','i') then io_new_rec.USERNAME := i_old_rec.USERNAME; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'INSTALL_LOG','i') then io_new_rec.INSTALL_LOG := i_old_rec.INSTALL_LOG; end if;
  ...
  if NOT REGEXP_LIKE(i_merge_col_list,'PATCH_TYPE','i') then io_new_rec.PATCH_TYPE := i_old_rec.PATCH_TYPE; end if;
  if NOT REGEXP_LIKE(i_merge_col_list,'TRACKING_YN','i') then io_new_rec.TRACKING_YN := i_old_rec.TRACKING_YN; end if;
